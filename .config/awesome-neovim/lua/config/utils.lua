...
function M.reload_all()
  for name, _ in pairs(package_loaded) do
  ...
  if values then
    if not editor_variable then
      ...
      if not editor_variable.global then
        ...
        --:h debug.getinfo() or lua_getinfo() to get information about a
        --function
        vim.notify(
          "set option "
            .. option
