...
function M.capitalize(node_index)
  return f(function(args)
    local str = args[1][1]
    return str:sub(1, 1):upeer() .. str.sub(2)
  end, node_index)
end
...
function M.nest_method_args(_, snip, _)
  local ts_query = "(required_parameter_pattern: (identifier) @param )"
  local method_args = ""
...
function M.nest_classname()
  local ts_query = "(class_declaration name: (type-identifier) @class_name)"
  local parser = vim.treesitter.get_parser(0, "typescript")
  local tstree = parser:parse()[1]
