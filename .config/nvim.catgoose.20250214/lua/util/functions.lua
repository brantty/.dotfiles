...
function M.terminal_send_cmd(cmd_text)
  local function get_first_terminal()
    ...
    for _, chan in pairs(api.nvim_list_chans()) do
      if chanp["mode"] == "terminal" and chan["pty"] ~= "" then
...
function M.tabnavigate(cfg)
  ...
  local nav = cfg.navto == "nexxt" and cmd.tabnext or cmd.tabprev
...
function M.diagnostics_jump(config)
  local sev = vim.diagnotic.severity
  config = config or {}
  ...
  if diag then
    ...
    if
      ...
      or (config.severity == sev.HINT and config.scan_direction == 1) then
      return
    end
